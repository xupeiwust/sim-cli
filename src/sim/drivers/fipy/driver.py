...
FiPy is NIST's pure-Python finite-volume PDE solver. Pip-installable
(`pip install fipy`); pure Python + numpy/scipy + (optionally) PySparse,
PyAMG, PyTrilinos, petsc4py.
...
_IMPORT_RE = re.compile(r"^\s*(import\s+fipy|from\s+fipy\b)", re.MULTILINE)
_USAGE_RE = re.compile(
    r"\b(CellVariable|FaceVariable|Grid1D|Grid2D|Grid3D|Mesh1D|Mesh2D|"
    r"Gmsh2D|Gmsh3D|TransientTerm|DiffusionTerm|ConvectionTerm|"
    r"ImplicitSourceTerm|ExplicitDiffusionTerm)\b|"
    r"\.solve\(|\.constrain\(|\.sweep\(",
)


def _probe_python_for_fipy(python_exe: Path) -> str | None:
    if not python_exe.is_file():
        return None
    try:
        proc = subprocess.run(
            [str(python_exe), "-c", "import fipy; print(fipy.__version__)"],
...
class FipyDriver:
    """Sim driver for FiPy (NIST pure-Python FVM PDE solver)."""
    ...
    def lint(self, script: Path) -> LintResult:
        ...
        if not _IMPORT_RE.search(text):
            diagnostics.append(Diagnostic(
                level="error",
                message="No `import fipy` / `from fipy` found",
            ))
        ...
        if _IMPORT_RE.search(text) and not _USAGE_RE.search(text):
            diagnostics.append(Diagnostic(
                level="warning",
                message=(
                    "No CellVariable / Grid / Term / .solve call — "
    ...
    def connect(self) -> ConnectionInfo:
        ...
        if not installs:
            return ConnectionInfo(
                solver="fipy", version=None, status="not_installed",
                message="fipy not importable from any known Python env",
            )
        top = installs[0]
        return ConnectionInfo(
            solver="fipy", version=top.version, status="ok",
            message=f"FiPy {top.version} in {top.path}",
    ...
    def detect_installed(self) -> list[SolverInstall]:
        ...
        def _record(python: Path, source: str) -> None:
            ver = _probe_python_for_fipy(python)
            if ver is None:
                return
    ...
    def run_file(self, script: Path):
        if script.suffix.lower() != ".py":
            raise RuntimeError(
                f"fipy driver only accepts .py scripts (got {script.suffix})"
            )
        installs = self.detect_installed()
        if not installs:
            raise RuntimeError("FiPy is not installed in any known Python env")
