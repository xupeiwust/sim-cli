"""pymoo driver for sim.

pymoo is the canonical Python framework for multi-objective optimization
(NSGA-II/III, MOEA/D, R-NSGA-III, U-NSGA-III, ...). Pip-installable
(`pip install pymoo`); pure Python + numpy + scipy + autograd.

Scripts are plain `.py`:
    from pymoo.algorithms.moo.nsga2 import NSGA2
    from pymoo.problems import get_problem
    from pymoo.optimize import minimize
    res = minimize(get_problem('zdt1'), NSGA2(pop_size=40), ('n_gen', 50))
"""
...
_IMPORT_RE = re.compile(r"^\s*(import\s+pymoo|from\s+pymoo\b)", re.MULTILINE)
_USAGE_RE = re.compile(
    r"\b(NSGA2|NSGA3|MOEAD|GA|DE|PSO|CMAES|RNSGA2|RNSGA3|UNSGA3|"
    r"get_problem|get_reference_directions|minimize|Problem|"
    r"ElementwiseProblem)\b",
)


def _probe_python_for_pymoo(python_exe: Path) -> str | None:
    if not python_exe.is_file():
        return None
    try:
        proc = subprocess.run(
            [str(python_exe), "-c", "import pymoo; print(pymoo.__version__)"],
...
class PymooDriver:
    """Sim driver for pymoo (multi-objective evolutionary optimization)."""
    ...
    def lint(self, script: Path) -> LintResult:
        ...
        if not _IMPORT_RE.search(text):
            diagnostics.append(Diagnostic(
                level="error",
                message="No `import pymoo` / `from pymoo` found",
            ))
        ...
        if _IMPORT_RE.search(text) and not _USAGE_RE.search(text):
            diagnostics.append(Diagnostic(
                level="warning",
                message=(
                    "No NSGA2 / Problem / minimize / get_problem call — "
    ...
    def connect(self) -> ConnectionInfo:
        ...
        if not installs:
            return ConnectionInfo(
                solver="pymoo", version=None, status="not_installed",
                message="pymoo not importable from any known Python env",
            )
        top = installs[0]
        return ConnectionInfo(
            solver="pymoo", version=top.version, status="ok",
            message=f"pymoo {top.version} in {top.path}",
    ...
    def detect_installed(self) -> list[SolverInstall]:
        ...
        def _record(python: Path, source: str) -> None:
            ver = _probe_python_for_pymoo(python)
            if ver is None:
                return
    ...
    def run_file(self, script: Path):
        if script.suffix.lower() != ".py":
            raise RuntimeError(
                f"pymoo driver only accepts .py scripts (got {script.suffix})"
            )
        installs = self.detect_installed()
        if not installs:
            raise RuntimeError("pymoo is not installed in any known Python env")
