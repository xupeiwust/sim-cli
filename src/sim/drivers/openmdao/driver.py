...
OpenMDAO is NASA's open-source framework for multidisciplinary design,
analysis, and optimization (MDAO). Pip-installable
(`pip install openmdao`); pure Python + numpy/scipy + networkx.
...
_IMPORT_RE = re.compile(
    r"^\s*(import\s+openmdao|from\s+openmdao\b)", re.MULTILINE,
)
_USAGE_RE = re.compile(
    r"\b(om|openmdao)\.(Problem|Group|ExplicitComponent|"
    r"ImplicitComponent|IndepVarComp|ExecComp|ScipyOptimizeDriver|"
    r"NonlinearBlockGS|DirectSolver|LinearBlockGS|NewtonSolver)\b|"
    r"\.add_subsystem\(|\.add_objective\(|\.add_design_var\(|"
    r"\.run_model\(|\.run_driver\(|\.setup\(",
)


def _probe_python_for_openmdao(python_exe: Path) -> str | None:
    if not python_exe.is_file():
        return None
    try:
        proc = subprocess.run(
            [str(python_exe), "-c",
             "import openmdao; print(openmdao.__version__)"],
...
class OpenMDAODriver:
    """Sim driver for OpenMDAO (NASA's MDAO framework)."""
    ...
    def lint(self, script: Path) -> LintResult:
        ...
        if not _IMPORT_RE.search(text):
            diagnostics.append(Diagnostic(
                level="error",
                message="No `import openmdao` / `from openmdao` found",
            ))
        ...
        if _IMPORT_RE.search(text) and not _USAGE_RE.search(text):
            diagnostics.append(Diagnostic(
                level="warning",
                message=(
                    "No om.Problem / Group / Component / .run_model call — "
    ...
    def connect(self) -> ConnectionInfo:
        ...
        if not installs:
            return ConnectionInfo(
                solver="openmdao", version=None, status="not_installed",
                message="openmdao not importable from any known Python env",
            )
        top = installs[0]
        return ConnectionInfo(
            solver="openmdao", version=top.version, status="ok",
            message=f"OpenMDAO {top.version} in {top.path}",
    ...
    def detect_installed(self) -> list[SolverInstall]:
        ...
        def _record(python: Path, source: str) -> None:
            ver = _probe_python_for_openmdao(python)
            if ver is None:
                return
    ...
    def run_file(self, script: Path):
        if script.suffix.lower() != ".py":
            raise RuntimeError(
                f"openmdao driver only accepts .py scripts (got {script.suffix})"
            )
        installs = self.detect_installed()
        if not installs:
            raise RuntimeError("OpenMDAO is not installed in any known Python env")
