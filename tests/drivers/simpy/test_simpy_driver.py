...
class TestDetect:
    def setup_method(self): self.d = SimpyDriver()
    def test_good(self): assert self.d.detect(FIXTURES / "simpy_good.py")
    def test_no_import(self): assert not self.d.detect(FIXTURES / "simpy_no_import.py")
...
class TestLint:
    def setup_method(self): self.d = SimpyDriver()
    def test_good(self): assert self.d.lint(FIXTURES / "simpy_good.py").ok
    def test_no_import(self): assert not self.d.lint(FIXTURES / "simpy_no_import.py").ok
    def test_no_usage(self):
        r = self.d.lint(FIXTURES / "simpy_no_usage.py")
        assert r.ok and any(x.level == "warning" for x in r.diagnostics)
    def test_syntax(self): assert not self.d.lint(FIXTURES / "simpy_syntax_error.py").ok
...
class TestConnect:
    ...
    def test_found(self, monkeypatch):
        ...
        monkeypatch.setattr(d, "detect_installed", lambda: [SolverInstall(
            name="simpy", version="4.0", path="/x", source="t",
...
class TestRunFile:
    def test_raises_when_not_installed(self, monkeypatch):
        d = SimpyDriver()
        monkeypatch.setattr(d, "detect_installed", lambda: [])
        with pytest.raises(RuntimeError, match="(?i)simpy"):
            d.run_file(FIXTURES / "simpy_good.py")
